...
import { useConfiguracao } from '../context/ConfiguracaoContext'; // Importando o hook
...
// --- Componente Cabecalho modificado ---
// Agora ele consome o contexto para obter a URL da logo.
export const Cabecalho: React.FC<CabecalhoProps> = ({ template, disciplina, serie, turma }) => {
  const { config } = useConfiguracao(); // Usando o hook para acessar as configurações

  return (
    <header className="border-b border-gray-300 pb-2 mb-4 flex-shrink-0">
      <div className="flex justify-between items-center">
        <div className="h-20 w-20 bg-gray-200 flex items-center justify-center text-xs text-gray-500 overflow-hidden">
          {config.logoUrl ? (
            <img src={config.logoUrl} alt="Logo da Instituição" className="max-h-full max-w-full object-contain" />
          ) : (
            <span>Logo</span>
          )}
        </div>
...
// --- Componente Rodape (sem alterações) ---
...
// --- Componente PaginaLayout (sem alterações) ---
